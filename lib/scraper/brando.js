...
function hasProgressChanged(previousItems, nextItems) {
  const nextProjectSlugs = new Set(nextItems.map((item) => item.project_slug))

  const hasChangedVisibleItems = nextItems.some((item) => {
    const previous = previousItems.get(item.project_slug)
    return (
      !previous ||
  ...
  return Array.from(previousItems.values()).some(
    (item) => item.is_visible === true && !nextProjectSlugs.has(item.project_slug),
  )
}
